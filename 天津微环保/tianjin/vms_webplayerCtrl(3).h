...
public:
	...
	*参  数：
		serverUuid					uuid
		play_session				由 StartVodStream 返回的句柄 固定值：1
	*返回值：
		int							成功/失败
	*备  注：
	*************************************************************************************************/		
...
//VOD
	...
	*参  数：
		...
		serverUuid					uuid
		devicePort					设备端口号
		strIP						设备ip    28181服务器注册的【通道编号】 例如测试海康平台为【34020000001330000001】
		deviceUser					设备用户名
		devicePwd					设备密码
		dwDeviceID					设备Id
		dwReserved
		DeviceCnts					子设备数
		DeviceType					设备类型  固定值：0x03
		dwCameraID					摄像头id
		dChannelID					通道id
		vodMode						播放模式  固定值：1
		enumFileType				文件类型  固定值：0
		startTime					开始时间
		endTime						结束时间
	*返回值：
		BSTR						句柄
	...
	*参  数：
		...
		startTime					开始时间
		endTime						结束时间
		dold_sessionid              文件编号（0开始） FindFile检索结果的编号             
		serveruuid					uuid
		dReserved
	*返回值：
		BSTR						播放句柄
	...
	*参  数：
		winID						窗口编号
		serverUuid					uuid
		video_path					摄像头名称（设备名+摄像头编号或通道号）
		state						状态 0:播放 1：停止
	...
	*备  注：
	*************************************************************************************************/
	LONG StartRecorde(LONG winID,LPCTSTR serverUuid,LPCTSTR video_path, LONG state);
